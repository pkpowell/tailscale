...
func (ns *Impl) Start() error {
	...
	ns.tundev.PreFilterOutNetstack = ns.handleLocalPackets
	return nil
}
...
func (ns *Impl) handleLocalPackets(p *packet.Parsed, t *tstun.Wrapper) filter.Response {
	// If its not traffic to the service IP (ie: magicDNS) we don't
...
// shouldProcessInbound reports whether an inbound (wg -> us) packet should be
// handled by netstack.
...
// injectInbound is installed as a packet hook on the 'inbound' (wg -> us)
// path. Returning filter.Accept releases the packet to continue normally
// (typically being delivered to the host networking stack), whereas returning
// filter.DropSilently is done when netstack intercepts the packet and no
// further processing towards to host should be done.
...
func (ns *Impl) acceptTCP(r *tcp.ForwarderRequest) {
	...
	if reqDetails.LocalPort == 53 && dialIP == magicDNSIP {
