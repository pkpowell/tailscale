...
type userspaceEngine struct {
	...
	icmpEchoCallback    map[uint32]func()                      // for ICMP echo responses
...
func NewUserspaceEngine(logf logger.Logf, conf Config) (_ Engine, reterr error) {
	...
	e.tundev.OnICMPEchoReceived = func(p *packet.Parsed) bool {
		idSeq := p.EchoIDSeq()
		e.mu.Lock()
		defer e.mu.Unlock()
		cb := e.icmpEchoCallback[idSeq]
		if cb == nil {
...
func (e *userspaceEngine) Ping(ip netaddr.IP, pingType string, cb func(*ipnstate.PingResult)) {
...
func (e *userspaceEngine) sendICMPEcho(ip netaddr.IP, peer *tailcfg.Node, res *ipnstate.PingResult, cb func(*ipnstate.PingResult)) {
	...
	expireTimer := time.AfterFunc(10*time.Second, func() {
		e.setICMPEchoCallback(idSeq, nil)
	})
	t0 := time.Now()
	e.setICMPEchoCallback(idSeq, func() {
		expireTimer.Stop()
		d := time.Since(t0)
...
func (e *userspaceEngine) setICMPEchoCallback(idSeq uint32, cb func()) {
	e.mu.Lock()
	defer e.mu.Unlock()
	if e.icmpEchoCallback == nil {
		e.icmpEchoCallback = map[uint32]func(){}
	}
	if cb == nil {
		delete(e.icmpEchoCallback, idSeq)
	} else {
		e.icmpEchoCallback[idSeq] = cb
	}
}
